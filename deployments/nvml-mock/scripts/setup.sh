...
# 7. Node labels. Both keys nvml-mock causes to exist are handled by
#    nvml-mock-node, which talks to the API server with client-go — this image
#    ships no kubectl. It writes nvidia.com/gpu.present directly and converges
#    the NFD feature file that yields
#    feature.node.kubernetes.io/pci-10de.present, gated on MOCK_NFD_PCI_LABEL.
#    See pkg/node/labels for why NFD's own PCI source cannot derive that key as
#    deployed, and why the file (not the label) is what we write (#505).
#
#    Cluster-side failures are warnings there, not exits: under `set -e` a
#    failure would abort this script before step 8's /host/run/nvidia/driver
#    symlink and crash-loop the whole mock for an optional label. An invalid
#    MOCK_NFD_PCI_LABEL value IS fatal, and that non-zero exit is meant to stop
#    the pod here rather than silently disable the gate.
nvml-mock-node label \
  --node-name "$NODE_NAME" \
  --pci-vendor-label "${MOCK_NFD_PCI_LABEL:-on}"
