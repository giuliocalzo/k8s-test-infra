...
### Changed
- The nvml-mock image no longer ships `kubectl`. The one label it wrote directly,
  `nvidia.com/gpu.present`, is now added and removed through `client-go` in a new
  `nvml-mock-node` binary: `nvml-mock-node label` (called from `setup.sh`) and
