...
#
# Usage: hack/changelog-fold.sh X.Y.Z
#   CHANGIE=/path/to/changie hack/changelog-fold.sh 0.4.0   # override the binary
#
# Pass the version unprefixed: CHANGELOG.md headings read "## [0.3.0]" while the
# matching git tag is v0.3.0.
#
...
if grep -q '^## \[Unreleased\]' "$CHANGELOG"; then
  echo "$CHANGELOG has an '## [Unreleased]' section — move those entries into .changes/unreleased/ fragments first" >&2
  exit 1
fi
